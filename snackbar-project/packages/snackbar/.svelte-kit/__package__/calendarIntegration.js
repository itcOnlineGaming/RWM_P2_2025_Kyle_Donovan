 ...
 */
export function notifyEventCreated(event, options) {
    ...
    if (options?.showDesktopNotification !== false && isNotificationGranted()) {
        sendNotification('Event Created', `${event.title} scheduled for ${dateStr}`, {
 ...
 */
export function notifyEventUpdated(event, options) {
    const dateStr = event.date.toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
        hour: 'numeric',
    ...
    if (options?.showDesktopNotification !== false && isNotificationGranted()) {
        sendNotification('Event Updated', `${event.title} has been updated`, {
            icon: '🔄',
 ...
 */
export function notifyEventCancelled(event, options) {
    snackbar.error(`Event cancelled: ${event.title}`, {
        position: options?.snackbarPosition || 'bottom-right',
        duration: 5000
    });
    if (options?.showDesktopNotification !== false && isNotificationGranted()) {
        sendNotification('Event Cancelled', `${event.title} has been cancelled`, {
            icon: '❌',
 ...
 */
export function notifyEventReminder(event, minutesBefore, options) {
    ...
    if (options?.showDesktopNotification !== false && isNotificationGranted()) {
        sendNotification('Meeting Reminder', `${event.title} starts at ${timeStr} (in ${minutesBefore} minutes)`, {
            icon: '⏰',
 ...
 */
export function notifyCalendarSynced(eventCount, options) {
    snackbar.success(`Calendar synced with ${eventCount} events`, {
        position: options?.snackbarPosition || 'bottom-center',
        duration: 3000
    });
    if (options?.showDesktopNotification !== false && isNotificationGranted()) {
        sendNotification('Calendar Synced', `${eventCount} events updated`, {
            icon: '🔄',
            tag: 'calendar-sync',
            silent: true
        });
